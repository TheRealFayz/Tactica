...
Tactica.DefaultData = {
    ...
    ["Upper Karazhan Halls"] = {
        ...
        ["Sanv Tas'dal"] = {
            ["Default"] = "Tanks: 3–4 tanks. MT holds boss at top of stairs facing away from raid. OT tanks adds from left/right portals when spawned, optional tank for mid portal at melee. During add phase boss untanked; all tanks help kill/tank adds during this phase, prio large.\nDPS: No decurse to reveal shades. If you see shades, kill them. All range stand lower level center and DPS prio adds from portals as they spawn, big first. Melee behind boss, but during add phase all on adds at lower center. Move when boss does AoE melee.\nHealers: Stand center lower ground (with range DPS). Heal MT at stairs and OTs at portals. Watch for heavy AoE melee dmg or from add. Do not decurse (it reveals shades).\nClass Specific: 2 Hunters rotate Tranq Shot on boss when needed. No one decurse to keep shades visible. Melee can cleave mid-portal adds at boss.\nBoss Ability: AoE melee dmg—melee move out. Spawns shades only visible with debuff. Add waves from 3 portals, large adds most dangerous. During add phase boss inactive."
        },
        ["Kruul"] = {
            ["Default"] = "Tanks: 5–6 tanks. 2 front (facing boss at start), 2 back (behind boss at start), 1 infernal tank (full FR), 1 add helper if needed. Taunt swap between front/back at 6 stacks (no more). Infernal tank left, DPS right. Boss ignore armor; so stack HP/threat.\nDPS: Ranged on boss only. Melee in front/back groups to soak cleave (~8+tanks in each group). Melee have good health. At 30% after knockback all melee chain LIP shouts/taunts, then die; ranged continues. Ignore infernals.\nHealers: Heal tanks/front/back groups. At 30% phase, let melee die after LIP taunt, focus ranged + tanks. One assigned decurser that removes decruse only after target moves from raid.\nClass Specific: Assign 1 decurser for Kruul’s curse. Melee tanks use LIP in 30% phase after knockback. Infernal tank uses full FR. Fury prot viable—boss ignores armor.\nBoss Ability: Cleave on front/back groups, stacking debuff (swap at 6). Summons infernals. At 30% gains 4× dmg. Casts decursable curse—must be decursed outside raid (assign a decurser - have player move out when getting decursed)."
        },
        ["Rupturan the Broken"] = {
            ["Default"] = "Tanks: 5 heavy + 2–3 OT. During P1-2 tanks on boss, 1 per add in corners. Always have a tank 2nd boss threat and 15y away to soak (run in and taunt swap to ensure). During P2-2 tanks per fragment (1+2 threat). 1-2 on Tanks Exiles.\nDPS: During P1 kill adds first—kill all adds at same time. Avoid add death explosions (think Garr adds). Dont overtake 1-2 tank threat on boss. During P2 nuke heart before full mana, then fragments to same % before killing. Kill Exiles asap.\nHealers: Stack center P1-P2 with Range. Watch tank burst + add explosions + Ouro tail damage. Dispel tank debuffs instantly in P2. Keep OT/soak tanks alive. Heal during kiting trails.\nClass Specific: Threat control—keep assigned tanks 1+2 on boss/frags. Dispel tanks fast. Avoid trail on ground. Spread for add explosions.\nBoss Ability: Add explosions on death, soak mechanic for 2nd threat tank, trail to kite, heart regen in P2, fragments require dual-threat tanks, Exile spawns."
        },
        ["Mephistroth"] = {
            ["Default"] = "Tanks: 3 tanks. MT on boss + 1 doomguard when boss teleports. 2nd on other doomguard + adds. 3rd helps pick ranged adds. Drag nightmare crawlers from ranged/healers as they soak mana. Doomguards AOE, so also move out. MT usually stationary-face boss away.\nDPS: Prio shards > adds > boss. Kill nightmare crawlers fast, drag from ranged. During shard phase, assigned 4–5 kill each Hellfury Shard in time limit. They spawn in the outter circle, with equal distance. Think of it like a clock.\nHealers: Stack with ranged. Heal shard teams. Watch for fear + burst on tank swap. Assign 2-3 dispellers, to cover shard groups on far side during this ability. Dispell immediately.\nClass Specific: No movement during Shackles—any movement wipes raid. Assigned groups kill Hellfury Shards fast. Drag nightmare crawlers out. Assign a few dispellers to also spread out during shards.\nBoss Ability: Shackles—no one moves or wipe. Hellfury Shards—kill fast. Spawns nightmare crawlers (mana drain) + doomguards. Fears raid. Dispell prio - not dispelling will cause a kills from center."
        }
    },
    ["Onyxia's Lair"] = {
        ["Onyxia"] = {
